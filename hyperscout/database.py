    ...
    async with sqlite3.connect(DATABASE_PATH) as conn:
        await conn.execute('''
            CREATE TABLE IF NOT EXISTS guilds (
                ...
                delete_after_minutes INTEGER DEFAULT 1440
            )
        ''')
        await conn.commit()
...
async def migrate_schema():
    ...
    async with sqlite3.connect(DATABASE_PATH) as conn:
        ...
        if 'delete_after_minutes' not in columns:
            await conn.execute('ALTER TABLE guilds ADD COLUMN delete_after_minutes INTEGER DEFAULT 1440')
        await conn.commit()

async def configure_guild(guild_id: str, destination_channel_id: str, spam_protection_minutes: int, delete_after_minutes: int):
    """Adds or updates a guild configuration in the database."""
    async with sqlite3.connect(DATABASE_PATH) as conn:
        await conn.execute('''
            INSERT OR REPLACE INTO guilds (guild_id, destination_channel_id, spam_protection_minutes, delete_after_minutes)
            VALUES (?, ?, ?, ?)
        ''', (guild_id, destination_channel_id, spam_protection_minutes, delete_after_minutes))
        await conn.commit()
...
async def get_all_guilds() -> List[Tuple[str, str, int, int]]:
    ...
    async with sqlite3.connect(DATABASE_PATH) as conn:
        async with conn.execute('SELECT guild_id, destination_channel_id, spam_protection_minutes, delete_after_minutes FROM guilds') as cursor:
...
async def get_guild_by_id(guild_id: int) -> Optional[Tuple[str, str, int, int]]:
    ...
    async with sqlite3.connect(DATABASE_PATH) as conn:
        async with conn.execute('SELECT guild_id, destination_channel_id, spam_protection_minutes, delete_after_minutes FROM guilds WHERE guild_id = ?', (str(guild_id),)) as cursor:

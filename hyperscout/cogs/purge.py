...
async def purge_bot_messages(bot: commands.Bot):
    ...
    for guild_id, destination_channel_id, _, delete_after_minutes in guilds:
        ...
        try:
            ...
            logger.info(
                f"Purging messages since {cutoff_date} from channel {destination_channel_id} in guild {guild_id}.",
                extra={
            ...
            purged_messages = await channel.purge(before=cutoff_date, check=lambda m: m.author == bot.user)
            logger.info(
...
class PurgeCog(commands.Cog):
    def __init__(self, bot):
        ...
        self.scheduler.add_job(self.trigger_purge, 'cron', hour=0, minute=0)
        self.scheduler.start()

    async def trigger_purge(self):
        await purge_bot_messages(self.bot)
